<?php $conn = new mysqli("usejs.top", "semen574_user", "2017semen574_", "semen574_garrage");
...
if ($action == 'completedtask') {
    ...
    $result = $conn->query("UPDATE `todotasks` SET `iscomplete` = '$iscomplete'  WHERE `id` = '$taskid'");
    if ($result) {
        $res['message'] = "status is changed";
    } else {
        $res['error'] = true;
        $res['message'] = "status is changed";
    }
